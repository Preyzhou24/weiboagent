...
import { getExternalAgentMdIncludes, getMemoryFiles, shouldShowAgentMdExternalIncludesWarning } from './utils/agentmd.js';
...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], claudeInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  // and checks AGENT.md external includes. bypassPermissions mode
  ...
  if (!isEnvTruthy(process.env.CLAUBBIT)) {
    ...
    if (await shouldShowAgentMdExternalIncludesWarning()) {
      const externalIncludes = getExternalAgentMdIncludes(await getMemoryFiles(true));
      const {
        AgentMdExternalIncludesDialog
      } = await import('./components/AgentMdExternalIncludesDialog.js');
      await showSetupDialog(root, done => <AgentMdExternalIncludesDialog onDone={done} isStandaloneDialog externalIncludes={externalIncludes} />);
    }
  }
