 ...
 *
 * 1. Managed memory (eg. /etc/claude-code/AGENT.md) - Global instructions for all users
 * 2. User memory (~/.claude/AGENT.md) - Private global instructions for all projects
 * 3. Project memory (AGENT.md, .claude/AGENT.md, and .claude/rules/*.md in project roots) - Instructions checked into the codebase
 ...
 * - AGENT.md, .claude/AGENT.md, and all .md files in .claude/rules/ are checked in each directory for Project memory
 *
...
/**
 * Checks whether an AGENT.md file path is excluded by the agentMdExcludes setting.
...
export async function processMemoryFile(
  ...
  // Skip if path is excluded by agentMdExcludes setting
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    // checked-in files like AGENT.md and .claude/rules/*.md, so the same
    // content gets loaded twice. Skip Project-type (checked-in) files from
    // directories above the worktree but within the main repo — the worktree
    // already has its own checkout. AGENT.local.md is gitignored so it only
    ...
    for (const dir of dirs.reverse()) {
      ...
      // Try reading AGENT.md (Project) - only if projectSettings is enabled
      if (isSettingSourceEnabled('projectSettings') && !skipProject) {
        ...
        // Try reading .claude/AGENT.md (Project)
        const dotClaudePath = join(dir, '.claude', 'AGENT.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
            'Project',
      ...
      // Try reading AGENT.local.md (Local) - only if localSettings is enabled
      if (isSettingSourceEnabled('localSettings')) {
        const localPath = join(dir, 'AGENT.local.md')
        result.push(
          ...(await processMemoryFile(
            localPath,
            'Local',
    ...
    // Process AGENT.md from additional directories (--add-dir) if env var is enabled
    // This is controlled by CLAUDE_CODE_ADDITIONAL_DIRECTORIES_AGENT_MD (legacy: CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD) and defaults to off
    // Note: we don't check isSettingSourceEnabled('projectSettings') here because --add-dir
    // is an explicit user action and the SDK defaults settingSources to [] when not specified
    if (isEnvTruthy(process.env.CLAUDE_CODE_ADDITIONAL_DIRECTORIES_AGENT_MD ?? process.env.CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD)) {
      const additionalDirs = getAdditionalDirectoriesForAgentMd()
      for (const dir of additionalDirs) {
        // Try reading AGENT.md from the additional directory
        const projectPath = join(dir, 'AGENT.md')
        result.push(
          ...(await processMemoryFile(
            projectPath,
        ...
        // Try reading .claude/AGENT.md from the additional directory
        const dotClaudePath = join(dir, '.claude', 'AGENT.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
    ...
    // memory system, not "instructions" in the AGENT.md/rules sense.
 ...
 * Loads AGENT.md, unconditional rules, and conditional rules for that directory.
 *
...
export async function getMemoryFilesForNestedDirectory(
  ...
  // Process project memory files (AGENT.md and .claude/AGENT.md)
  ...
  // Process local memory file (AGENT.local.md)
  if (isSettingSourceEnabled('localSettings')) {
    const localPath = join(dir, 'AGENT.local.md')
    result.push(
...
/**
 * Check if a file path is a memory file (AGENT.md, AGENT.local.md, or .claude/rules/*.md)
 */
export function isMemoryFilePath(filePath: string): boolean {
  const name = basename(filePath)

  // AGENT.md or AGENT.local.md anywhere
  if (name === 'AGENT.md' || name === 'AGENT.local.md') {
    return true
  }
